...
import { Compass, SlidersHorizontal, BarChart2 } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-base">
        ...
        <div className="mx-auto flex max-w-5xl gap-1 px-6">
          <button onClick={() => setTab('analyze')}
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-medium transition ${
              tab === 'analyze' ? 'border-gold text-gold' : 'border-transparent text-muted hover:text-ink'
            }`}>
            <BarChart2 size={15} />
            Analyze
          </button>
          <button onClick={() => setTab('screener')}
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-medium transition ${
              tab === 'screener' ? 'border-gold text-gold' : 'border-transparent text-muted hover:text-ink'
            }`}>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
            ...
            {!data && !loading && !error && (
              <div className="rounded-sm border border-dashed border-hairline p-10 text-center">
                ...
                <p className="mt-2 text-sm text-muted">
                  Price stats, buy/sell timing, AI sentiment, and a call — all in one view.
                </p>
              </div>
            )}
            {loading && (
              <div className="rounded-sm border border-hairline bg-panel p-10 text-center text-sm text-muted">
                Pulling price history, scanning headlines, asking the model for a read…
              </div>
            )}
            {error && (
        ...
        {/* ── Screener Tab ── */}
        {tab === 'screener' && (
          <Screener onAnalyze={handleSearch} />
        )}
