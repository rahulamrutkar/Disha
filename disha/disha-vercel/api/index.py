...
def _ensure_yf_session() -> bool:
    ...
    try:
        ...
        s.get("https://finance.yahoo.com/", headers=YF_HEADERS, timeout=10)
        # Get crumb
...
def screener(
    ...
    # Only runs if user set a market cap or P/E filter. Uses parallel fetch.
    need_fundamentals = min_market_cap_cr > 0 or max_pe < 9999
    if need_fundamentals and candidates:
        with ThreadPoolExecutor(max_workers=5) as ex:
            fund_map = {ex.submit(fetch_fundamentals, s["symbol"]): s for s in candidates}
            for future in as_completed(fund_map, timeout=20):
    ...
    # Missing data (None) = INCLUDE the stock, show "—" to the user
    results: list[dict] = []
    for s in candidates:
