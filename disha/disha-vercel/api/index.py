...
def fetch_fundamentals(symbol: str) -> dict:
    """
    Phase 2 — quoteSummary with summaryDetail module.
    summaryDetail contains marketCap + trailingPE (price module does NOT have PE).
    Falls back to defaultKeyStatistics if trailingPE missing from summaryDetail.
    """
    nse = to_nse(symbol)
    ck  = f"fund2:{nse}"
    now = time.time()
