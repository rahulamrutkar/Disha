...
def fetch_stock_price_trend(symbol: str) -> dict | None:
    """
    Phase 1 — uses the same v8/finance/chart endpoint as the Analyze feature.
    Known to work for individual and parallel calls. Gets: price, trend, day change.
    """
    nse = to_nse(symbol)
    ck  = f"pt:{nse}"
    now = time.time()
    ...
    try:
        ...
        # Compute 20-day SMA from price series for trend
        q      = chart.get("indicators", {}).get("quote", [{}])[0]
        ...
        m = {
            ...
            "pe":            None,
            "revenue_growth": None,
            "trend":         ("bullish" if price > sma20 else "bearish") if sma20 else "unknown",
        }
        _cache[ck] = (now, m)
        return m
    except Exception as e:
...
def fetch_fundamentals(symbol: str) -> dict:
    """
    Phase 2 — fetches P/E and market cap via quoteSummary price module.
    Only called for stocks that pass Phase 1 filters (much smaller set).
    Uses query2 and only the light price module.
    """
    nse = to_nse(symbol)
    ck  = f"fund:{nse}"
    now = time.time()
    if ck in _cache and now - _cache[ck][0] < 900:
        return _cache[ck][1]
    try:
        url = f"https://query2.finance.yahoo.com/v10/finance/quoteSummary/{quote_plus(nse)}"
        r   = requests.get(url, params={"modules": "price"},
                           headers=YF_HEADERS, timeout=8)
        r.raise_for_status()
        pd_ = r.json().get("quoteSummary", {}).get("result", [{}])[0].get("price", {})
        mkt = _raw(pd_, "marketCap")
        pe  = _raw(pd_, "trailingPE")
        result = {
        ...
        return {"market_cap_cr": None, "pe": None}
...
def screener_debug():
    ...
    return {
        ...
        "samples":       samples,
        "note": "Check trend/price/pe/market_cap_cr values to understand filter behaviour",
    }
