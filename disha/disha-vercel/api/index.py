...
def fetch_chart(nse_symbol: str, interval: str = "1d", range_: str = "1mo") -> dict:
    # Cache at the chart level — avoids duplicate YF calls within same request
    ck  = f"chart:{nse_symbol}:{interval}:{range_}"
    now = time.time()
    ttl = 300 if interval == "1d" else 120   # daily data cached 5 min, intraday 2 min
    ...
    for host in ("query1", "query2"):
        try:
            ...
            r   = requests.get(url, params=params, headers=YF_HEADERS, timeout=10)
            r.raise_for_status()
            data   = r.json()
            result = data.get("chart", {}).get("result")
            if result:
                _cache[ck] = (now, result[0])
                return result[0]
...
def _combined_ai(symbol: str, company: str, stats: dict, tech: dict,
                 timing: dict, headlines: list[str]) -> dict:
    """
    Single Gemini call that returns BOTH sentiment analysis AND buy/sell call.
    Halves the Gemini API latency vs two separate calls.
    """
    numbered = "\n".join(f"{i+1}. {h}" for i, h in enumerate(headlines[:6]))
    prompt = f"""You are a financial analyst for Indian stocks (NSE/BSE).

Company: {company} ({symbol})
30-day stats: {json.dumps(stats)}
Technical: {json.dumps(tech)}
Recent headlines:
{numbered if numbered else "No recent news."}
...
{{
  "sentiment": {{
    ...
    "summary": "<2 sentence summary of news impact>",
    "key_events": ["<event1>", "<event2>"],
    "per_headline": [{{"headline":"<text>","sentiment":"positive|negative|neutral","reason":"<short>"}}]
  }},
  "call": {{
    "call": "BUY|SELL|HOLD",
    "confidence": <0-100>,
    "rationale": "<2-3 sentences combining technical + news>",
    "key_risks": ["<risk1>", "<risk2>"],
    "disclaimer": "Educational demo only. Not personalized financial advice."
  }}
}}"""

    result = call_gemini(prompt)
    if result and isinstance(result, dict) and "sentiment" in result and "call" in result:
        result["sentiment"]["ai_available"] = True
        result["call"]["ai_available"]      = True
        return result

    # Fallback: rule-based call
    score = tech.get("score", 0)
    call  = "BUY" if score >= 1 else "SELL" if score <= -1 else "HOLD"
    return {
        "sentiment": {"overall_sentiment":"unknown","sentiment_score":0,
                      "summary":"AI analysis unavailable — add GEMINI_API_KEY to Vercel env vars.",
                      "key_events":[],"per_headline":[],"ai_available":False},
        "call":      {"call":call,"confidence":40,
                      "rationale":"Rule-based fallback: AI narrative unavailable.",
                      "key_risks":["AI unavailable"],"disclaimer":"Not financial advice.",
                      "ai_available":False},
    }
...
def analyze(symbol: str):
    """
    Optimized: ONE Yahoo Finance chart call fetches price+company+quote+series.
    Then parallel: intraday + news simultaneously.
    Then ONE combined Gemini call.
    Total: 2 YF calls + 1 Google News + 1 Gemini = well within 10s limit.
    """
    symbol = symbol.strip().upper().replace(".NS", "").replace(".BO", "")
    try:
        nse = to_nse(symbol)

        # ── Single chart call: gets price series + company name + quote ──────
        chart = fetch_chart(nse, "1d", "3mo")
        meta  = chart.get("meta", {})
        ...
        change_pct = round((change / float(prev)) * 100, 2) if prev and change else None
        quote = {
        ...
        # ── Parallel: intraday + news ─────────────────────────────────────────
        def _intraday():
            try:    return get_intraday_rows(symbol, days=20)
            except: return []

        def _news(co):
            try:    return fetch_news(co)
            except: return []

        with ThreadPoolExecutor(max_workers=2) as ex:
            f_int  = ex.submit(_intraday)
            f_news = ex.submit(_news, company)
            intraday_rows = f_int.result(timeout=7)
            news_items    = f_news.result(timeout=7)

        stats  = basic_stats(rows)
        tech   = technical_signal(rows)
        timing = (timing_pattern(intraday_rows) if intraday_rows
                  else {"available": False, "grid": [], "best_buy": None, "best_sell": None})

        # ── Single combined Gemini call ───────────────────────────────────────
        headlines = [n["title"] for n in news_items]
        combined  = _combined_ai(symbol, company, stats, tech, timing, headlines)
        sentiment = combined.get("sentiment", {
            "overall_sentiment": "unknown", "sentiment_score": 0,
            "summary": "", "key_events": [], "per_headline": [], "ai_available": False,
        })
        call_data = combined.get("call", {
            "call": "HOLD", "confidence": 40,
            "rationale": "AI unavailable.", "key_risks": [],
            "disclaimer": "Not financial advice.", "ai_available": False,
        })
...
def market_overview():
    """Indices + top movers + sector breadth for the Market Overview page."""
    # ── 1. Indices ────────────────────────────────────────────────────────────
    INDEX_MAP = {
        "NIFTY 50":   "^NSEI",
        "SENSEX":     "^BSESN",
    ...
    for name, sym in INDEX_MAP.items():
        ...
        try:
            ...
            print(f"[overview idx] {name}: {e}")
            indices.append({"name": name, "symbol": sym, "price": None, "change": None,
                            "change_pct": None, "sparkline": [], "is_vix": name == "INDIA VIX"})

    # ── 2. Top movers from blue-chip universe (cached via fetch_screener_universe) ─
    MOVERS_UNIVERSE = ALL_SECTORS_STOCKS[:25]
    stocks = fetch_screener_universe(MOVERS_UNIVERSE)
    stocks_with_chg = [s for s in stocks if s.get("change_pct") is not None]
    stocks_sorted   = sorted(stocks_with_chg, key=lambda x: x.get("change_pct", 0), reverse=True)
    ...
    for s in stocks:
        ...
        if sec not in sector_map:
            sector_map[sec] = {"name": sec, "gainers": 0, "losers": 0, "total": 0, "sum_chg": 0.0}
        pct = s.get("change_pct") or 0.0
        sector_map[sec]["total"]   += 1
        sector_map[sec]["sum_chg"] += pct
        if pct >= 0:
            sector_map[sec]["gainers"] += 1
        else:
            sector_map[sec]["losers"]  += 1
    sectors = []
    for d in sector_map.values():
        avg = round(d["sum_chg"] / d["total"], 2) if d["total"] else 0
        sectors.append({"name": d["name"], "gainers": d["gainers"],
                        "losers": d["losers"], "total": d["total"], "avg_chg": avg})
    sectors.sort(key=lambda x: x["avg_chg"], reverse=True)

    # ── 4. Market breadth ─────────────────────────────────────────────────────
    advancing = sum(1 for s in stocks_with_chg if s["change_pct"] >= 0)
    declining = len(stocks_with_chg) - advancing
