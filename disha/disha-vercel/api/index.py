...
def fetch_stock_price_trend(symbol: str) -> dict | None:
    """
    Phase 1 — v8/finance/chart (same endpoint used by working Analyze feature).
    Fetches price, 20-day SMA trend, and computes day change from previousClose.
    """
    nse = to_nse(symbol)
    ck  = f"pt2:{nse}"
    now = time.time()
    ...
    try:
        ...
        # Change % — use previousClose from meta (reliable), fallback to last 2 closes
        prev = meta.get("previousClose") or meta.get("chartPreviousClose")
        ...
        m = {
            ...
            "change_pct":     chg_pct,
            "market_cap_cr":  None,   # filled by Phase 2
            "pe":             None,   # filled by Phase 2
            "revenue_growth": None,
            "trend":          trend,
        }
        _cache[ck] = (now, m)
...
# ── Yahoo Finance crumb session (needed for quoteSummary in 2025/2026) ─────────
import threading as _threading
_yf_session: dict = {"crumb": None, "cookies": {}, "ts": 0}
_yf_lock = _threading.Lock()   # prevents race condition when parallel workers all try to refresh

def _ensure_yf_session() -> bool:
    """
    Yahoo Finance now requires a crumb + cookies for quoteSummary calls.
    Step 1: visit finance.yahoo.com to get consent cookies.
    Step 2: call /v1/test/getcrumb to get the crumb token.
    Cached for 30 minutes so it only refreshes occasionally.
    """
    now = time.time()
    if _yf_session["crumb"] and now - _yf_session["ts"] < 1800:
        return True
    # Only ONE thread refreshes at a time — others wait and then reuse the result
    if not _yf_lock.acquire(timeout=5):
        return bool(_yf_session.get("crumb"))
    try:
        if _yf_session["crumb"] and time.time() - _yf_session["ts"] < 1800:
            return True  # another thread already refreshed while we waited
        s = requests.Session()
        s.get("https://finance.yahoo.com/", headers=YF_HEADERS, timeout=8)
        r = s.get("https://query1.finance.yahoo.com/v1/test/getcrumb",
                   headers=YF_HEADERS, timeout=8)
        crumb = r.text.strip()
        if crumb and 3 < len(crumb) < 60:
            _yf_session["crumb"]   = crumb
            _yf_session["cookies"] = dict(s.cookies)
            _yf_session["ts"]      = time.time()
            print(f"[crumb] refreshed: {crumb[:8]}…")
            return True
    except Exception as e:
...
def fetch_fundamentals(symbol: str) -> dict:
    """
    Fetch market cap + P/E for one stock.
    Tries 3 approaches in order — stops at the first that returns real data.

    Approach 1: v1/finance/quote (lightest, sometimes works without crumb)
    Approach 2: quoteSummary with crumb + cookies (proper auth)
    Approach 3: return nulls (screener still works, just shows — for those cols)
    """
    nse = to_nse(symbol)
    ck  = f"fund3:{nse}"
    now = time.time()
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if items:
                q   = items[0]
                res = _parse(q.get("marketCap"), q.get("trailingPE"))
    ...
    # ── Approach 3: give up, return nulls ─────────────────────────────────────
    _cache[ck] = (now, empty)
    return empty
...
def _screener_impl(
    ...
    # ── Phase 1: v8 chart for ALL stocks (fast, reliable) ────────────────────
    phase1: list[dict] = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        futures = {ex.submit(fetch_stock_price_trend, sym): sym for sym in universe}
    ...
    # ── Apply price + trend filters to narrow the candidate list ─────────────
    candidates: list[dict] = []
    for s in phase1:
        price = s.get("price")
        if price is None or not (min_price <= price <= max_price):
            continue
        # Unknown trend passes through (SMA not available → include anyway)
        if trend != "any" and s.get("trend") not in (trend, "unknown"):
            continue
        candidates.append(s)

    # ── Phase 2: fetch fundamentals (P/E, market cap) for candidates ────────
    # Wrapped in try-except — if Phase 2 crashes for any reason (timeout,
    # rate-limit, crumb failure) the screener still returns Phase 1 results.
    need_fundamentals = min_market_cap_cr > 0 or max_pe < 9999
    if need_fundamentals and candidates:
        try:
            with ThreadPoolExecutor(max_workers=4) as ex:
                fund_map = {ex.submit(fetch_fundamentals, s["symbol"]): s
                            for s in candidates}
            ...
            # Phase 2 failed entirely — continue with Phase 1 results, nulls for PE/cap
            print(f"[screener] Phase 2 failed: {e} — returning results without fundamentals")

    # ── Apply fundamental filters ─────────────────────────────────────────────
    # Rule: missing data (None) → INCLUDE the stock, show "—" to the user.
    # This ensures Phase 2 failures don't cause 0 results.
    results: list[dict] = []
    for s in candidates:
        cap = s.get("market_cap_cr")
        # Only exclude if we HAVE cap data AND it fails the filter
        if min_market_cap_cr > 0 and cap is not None and cap < min_market_cap_cr:
            continue
        pe = s.get("pe")
        # Only filter PE if we HAVE pe data AND user set a real max (not 9999)
...
def screener_debug():
    """Returns sample data for RELIANCE + SBIN so you can see what values filters compare against."""
    samples = {}
    for sym in ["RELIANCE", "SBIN", "TCS", "CANBK"]:
        pt   = fetch_stock_price_trend(sym)
    ...
    return {
        ...
        "crumb_ok":      bool(_yf_session.get("crumb")),
        "crumb_preview": (_yf_session.get("crumb") or "")[:10] + "…",
        "samples":       samples,
        "note": "crumb_ok=true means Yahoo Finance auth succeeded → PE/MktCap should now populate",
    }
