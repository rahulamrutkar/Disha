...
# Load .env for local development (no-op on Vercel where env vars are set in the dashboard)
try:
...
def call_gemini(prompt: str):
    ...
    models_to_try = ([preferred] if preferred and preferred not in GEMINI_FALLBACK_MODELS else []) + GEMINI_FALLBACK_MODELS
    ...
    for model in models_to_try:
        try:
            ...
            if r.status_code in (404, 429):
                print(f"Gemini {model} -> {r.status_code}, trying next model")
                continue
            r.raise_for_status()
            ...
            print(f"Gemini {model} HTTP {e.response.status_code}: {e.response.text[:120]}")
            continue
        except Exception as e:
            print(f"Gemini {model} error: {type(e).__name__}: {e}")
            continue

    print("Gemini: all models exhausted or failed")
    return None
...
{{"response": "<concise 2-3 sentence answer about Indian markets>",
  "disclaimer": "Not financial advice. Please verify independently."}}"""
        ...
        has_key = bool(os.getenv("GEMINI_API_KEY")) and os.getenv("GEMINI_API_KEY") != "your_gemini_api_key_here"
        if not has_key:
            return {"response": "⚠️ AI Copilot needs a Gemini API key to work. Add GEMINI_API_KEY to your .env file (get one free at https://aistudio.google.com/app/apikey), then restart the backend.", "disclaimer": ""}
        return {"response": "AI is temporarily unavailable. Please try again in a moment.", "disclaimer": ""}
    except Exception as e:
