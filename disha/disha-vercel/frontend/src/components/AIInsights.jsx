...
function MiniScore({ label, value, color }) {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
    </div>
  )
}
...
export default function AIInsights({ data }) {
  if (!data) return (
    <div className="card" style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12, minHeight: 300 }}>
      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)' }}>AI Insights</div>
    ...
    <div className="card" style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>AI Recommendation</div>
          <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 4 }}>{conf}% confidence</div>
        </div>
      </div>

      {/* Sub scores */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, background: 'var(--bg-hover)', borderRadius: 8, padding: '12px 8px' }}>
      ...
      {/* Rationale */}
      {call?.rationale && (
        <div style={{ fontSize: 12, color: 'var(--text-3)', lineHeight: 1.6, background: 'var(--bg-hover)', borderRadius: 8, padding: '10px 12px' }}>
          {call.rationale}
        </div>
      )}
      ...
      {call?.key_risks?.length > 0 && (
        <div>
          ...
          {call.key_risks.map((r, i) => (
            <div key={i} style={{ fontSize: 11, color: 'var(--text-3)', padding: '2px 0' }}>· {r}</div>
          ))}
        </div>
      )}

      {/* Disclaimer */}
      <div style={{ fontSize: 10, color: 'var(--text-3)', opacity: 0.7, lineHeight: 1.5 }}>
