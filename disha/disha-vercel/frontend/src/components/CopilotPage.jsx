...
export default function CopilotPage({ currentSymbol, data }) {
  ...
  useEffect(() => {
    fetch('/api/health').then(r => r.json()).then(d => setAiEnabled(d.ai_enabled)).catch(() => setAiEnabled(false))
  }, [])
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      ...
      {aiEnabled === false && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: '#2d1f00', border: '1px solid #a16207', borderRadius: 8, marginBottom: 14, fontSize: 12, color: '#fbbf24' }}>
          <AlertTriangle size={14} />
          <span>AI is disabled — <strong>GEMINI_API_KEY</strong> not set. Add it to your <code>.env</code> file and restart the backend. Get a free key at <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" style={{ color: '#fbbf24' }}>aistudio.google.com</a>.</span>
        </div>
      )}
