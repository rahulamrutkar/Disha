...
function CandleTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', fontSize: 12 }}>
      <div style={{ color: 'var(--text-3)', marginBottom: 6 }}>{d.date}</div>
...
export default function StockChart({ data, company, quote }) {
  ...
  if (!data?.length) return (
    <div className="card" style={{ flex: 1, minHeight: 340, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)', fontSize: 14 }}>
  ...
  return (
    <div className="card" style={{ padding: '20px 20px 16px', flex: 1 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div style={{ fontSize: 13, color: 'var(--text-3)', fontWeight: 500, marginBottom: 4 }}>{company || '—'} · NSE</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            <span style={{ fontSize: 28, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '-1px' }}>
      ...
      <div style={{ height: 260 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: -10 }}>
            ...
            <YAxis tick={{ fill: 'var(--text-3)', fontSize: 10, fontFamily: 'JetBrains Mono' }}
