    ...
    <div className="rounded-sm border border-hairline bg-panel p-5">
      <div className="flex items-center justify-between">
        ...
        {sentiment.overall_sentiment && (
          <span className={`rounded-sm border px-2 py-0.5 text-[11px] uppercase tracking-wide ${SENTIMENT_STYLE[sentiment.overall_sentiment] || SENTIMENT_STYLE.unknown}`}>
            {sentiment.overall_sentiment}
          </span>
        )}
      </div>
      ...
      {sentiment.key_events?.length > 0 && (
        <ul className="mt-3 flex flex-wrap gap-2">
          {sentiment.key_events.map((e, i) => (
            <li key={i} className="rounded-sm border border-hairline bg-panel2 px-2 py-1 text-xs text-goldsoft">{e}</li>
          ))}
        </ul>
      )}
        ...
        {news.map((item, i) => (
          <a key={i} href={item.link} target="_blank" rel="noopener noreferrer"
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-muted">{item.source}{item.reason ? ` · ${item.reason}` : ''}</p>
            </div>
            <div className="flex shrink-0 items-center gap-2">
              <span className={`whitespace-nowrap rounded-sm border px-1.5 py-0.5 text-[10px] uppercase ${SENTIMENT_STYLE[item.sentiment] || SENTIMENT_STYLE.unknown}`}>
                {item.sentiment}
              </span>
